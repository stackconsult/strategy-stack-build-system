...
async def test():
    ...
    async with agent.db_pool.acquire() as conn:
        ...
        await conn.execute(
            """INSERT INTO gates (gate_id, build_id, status, passed_by, evidence, passed_at)
               VALUES ('G-01', $1, 'PASSED', 'PO_AGENT_v1', '{}'::jsonb, NOW())""",
            build_id
        )
    ...
    # Verify G-02 was emitted
    async with agent.db_pool.acquire() as conn:
        gate = await conn.fetchrow("SELECT * FROM gates WHERE build_id = $1 AND gate_id = 'G-02'", build_id)
    assert gate is not None, "gate G-02 not emitted"
    print("✅ gate_emitted: PASS")
    
    # Verify awaited G-01
    assert result["awaited_gate"] == "G-01", "did not await G-01"
